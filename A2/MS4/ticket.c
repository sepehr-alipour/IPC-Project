...
void manageTicket(struct Ticket* tickets, struct Person person, int maxTicketLength) {
	int ticketNumber, i, option, indextTicket;
	char confirmation;
	printf("\nEnter ticket number: ");
	...
	do {
		...
		printf("Acct#   : %d\n", tickets[indextTicket].accountNumber);
		printf("Customer: %s\n", tickets[indextTicket].messages->fullName);
		printf("----------------------------------------\n");
		...
		case 1:
			...
			newMessage.type = 'C';
			strcpy(newMessage.fullName, tickets[indextTicket].messages->fullName);
